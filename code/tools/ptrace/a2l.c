...
translate_addresses (bfd *abfd, asection *section, char *addr_str)
{
    ...
    for (;;)
    {
        ...
        else
        {
            ...
            --naddr;
            */
            pc = bfd_scan_vma (addr_str, NULL, 16);
        //}
...
process_file (const char *file_name, const char *section_name,
	      const char *target, char *addr_str)
{
    bfd *abfd;
    asection *section;
    char **matching;
    ...
    translate_addresses (abfd, section, addr_str);
...
{
    char addrbuf[32]; //can not trans to const char* or will segmentfalt
    char *target = NULL;
    char *section_name = NULL;

    with_functions = TRUE;
    snprintf(addrbuf, sizeof(addrbuf), "%x", (unsigned int)_addr);
    //printf("addr=[%s] file:[%s]\n", addrbuf, file_name);

    return process_file (file_name, section_name, target, addrbuf);
}

#if 0
int
main (int argc, char **argv)
{
  ...
  printf("====naddr=%d===\n", naddr);
  int ia = 0;
  for (ia = 0; ia < naddr; ia++) {
      printf("====addr=%s==\n", addr[ia]);
  }

  char addrbuf[32];
  snprintf(addrbuf, sizeof(addrbuf), "%s", addr[0]);
  printf("%s %d -----file_name %s section_name %s target %s addrbuf %s-----------------------.\n", 
          __func__, __LINE__, file_name, section_name, target, addrbuf);
