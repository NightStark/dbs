...
static void set_config(const char *stf, const char *cb)
{
    ...
    //config.dhcp_statefile = strdup("/tmp/hosts/odhcpd");
    //config.dhcp_statefile = strdup("/tmp/odhcpd");
    config.dhcp_statefile = strdup(stf);
    /*
       ...
       */
    //config.dhcp_cb = strdup("/usr/sbin/odhcpd-update");
    config.dhcp_cb = strdup(cb);
}
...
int config_interface(const char *name, const char *ifname, const char *dns)
{
    //const char *prio = NULL;
    struct in6_addr *ipv6_dns = NULL;
    ...
    if (!iface) { 
        ...
        strncpy(iface->name, name, sizeof(iface->name) - 1); 
        list_add(&iface->head, &interfaces); 
        syslog(LOG_DEBUG, "[%s][%d]*****add --new-- iface******\n", __func__, __LINE__); 
    } 

    if (strncmp(name, "lan", 3) == 0) {
	    strncpy(iface->ifname, ifname, sizeof(iface->ifname) - 1);
		//strncpy(iface->ifname, "eno16777984", sizeof(iface->ifname) - 1);
        /*
        iface->dhcpv4_start.s_addr = 100;
        iface->dhcpv4_end.s_addr = 100 + 150;
        iface->dhcpv4_leasetime = 3600 * 4;
        */
        iface->dhcpv4 = RELAYD_DISABLED;
        iface->ra     = RELAYD_SERVER;
        iface->dhcpv6 = RELAYD_SERVER;
        //iface->master = 1;
        iface->managed = RELAYD_MANAGED_MFLAG;  /* RA mode */
        /* config ra prefix flags */
        iface->ra_advrouter= 0; 
        iface->ra_not_onlink = 0;

        iface->ra_maxinterval = 5;
        iface->ndp    = RELAYD_RELAY;
        //iface->default_router = 600; //?
        //iface->learn_routes = 1;
        //iface->external = 1;

        /* set dns config */
        ipv6_dns = malloc(sizeof(struct in6_addr));
        if (NULL == ipv6_dns) {
            syslog(LOG_ERR, "[%s][%d]*****malloc failed******\n", __func__, __LINE__);
            return -1;
        }
        //tc_str2ip6("2001:da8:20d:30::30", ipv6_dns);
        memset(ipv6_dns, 0, sizeof(struct in6_addr));
        tc_str2ip6(dns, ipv6_dns);
        iface->dns = ipv6_dns;
        iface->dns_cnt = 1;

        unsigned char buf[128];
        int len = dn_comp("lan", buf, sizeof(buf), NULL, NULL);
        iface->search = malloc(len);
        if (iface->search == NULL) {
            syslog(LOG_ERR, "[%s][%d]*******malloc failed****\n", __func__, __LINE__);
            return -1;
        }
        memset(iface->search, 0, len);
        iface->search_len = 0;
        memcpy(&iface->search[iface->search_len], buf, len);
        iface->search_len +=len;
    } 
    if (strncmp(name, "wan", 3) == 0) {
        ...
        //iface->master = 1;
        iface->managed = RELAYD_MANAGED_MFLAG;  /* RA mode */
        iface->ra_advrouter= 1; 
        iface->ra_maxinterval = 5;
        iface->ndp    = RELAYD_HYBRID;
        //iface->default_router = 600; //?

		strncpy(iface->ifname, "tun0", sizeof(iface->ifname) - 1);
#endif
    }

	iface->inuse = true;
    syslog(LOG_ERR, "[%s][%d]*****ifname:%s******\n", __func__, __LINE__, iface->ifname);
	if ((iface->ifindex = if_nametoindex(iface->ifname)) <= 0) {
		goto err;
    }
...
err:
    syslog(LOG_ERR, "config interfaces failed.");
	close_interface(iface);
	return -1;
}

static int set_interface(const char *ifname, const char *dns)
{
#if 0
	blob_buf_init(&b, 0);
	uci_to_blob(&b, s, &interface_attr_list);
	return config_parse_interface(blob_data(b.head), blob_len(b.head), "a name", true);
#endif
    config_interface("lan", ifname, dns);
    return 0;
}


void odhcpd_reload(const char *stf, 
                   const char *cb, 
                   const char *ifname, 
                   const char *dns)
{
...
void odhcpd_run(int argc, char *argv[])
{
    int c = 0;
    char dhcp_statefile[128] = {0};
    char dhcp_cb[128] = {0};
    char ifname[32] = {0};
    char dns[64] = {0};
#if 0
    ...
    while(1) {
        ...
        switch (c) {
            case 's':
                ...
                break;
            case 'c':
                snprintf(dhcp_cb, sizeof(dhcp_cb), "%s", optarg);
                break;
            case 'i':
                snprintf(ifname, sizeof(ifname), "%s", optarg);
                break;
            case 'd':
                snprintf(dns, sizeof(dns), "%s", optarg);
                break;
            default:
                syslog(LOG_ERR, "UNKNOWN para.\n");
                break;
        }
    }

    if (dhcp_statefile[0] == '\0') {
        snprintf(dhcp_statefile, sizeof(dhcp_statefile), "%s", "/tmp/hosts/odhcpd");
    }
    if (dhcp_cb[0] == '\0') {
        snprintf(dhcp_cb, sizeof(dhcp_cb), "%s", "/usr/sbin/odhcpd-update");
    }
    if (ifname[0] == '\0') {
        snprintf(ifname, sizeof(ifname), "%s", "br-lan");
    }
    if (dns[0] == '\0') {
        snprintf(dns, sizeof(dns), "%s", "2001:da8:20d:30::30");
    }

	odhcpd_reload(dhcp_statefile, dhcp_cb, ifname, dns);
	uloop_run();
