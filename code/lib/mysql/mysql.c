#include <stdio.h>
#include <stdlib.h>
#include <mysql/mysql.h>  //我的机器上该文件在/usr/include/mysql下
...
int mysql_main(char *argv) 
{
    ...
    sprintf(qbuf,SELECT_QUERY,atoi(argv));
    ...
    while (row = mysql_fetch_row(res)) {
        printf("Ther userid #%d 's username is: %s\n", atoi(argv),(((row[0]==NULL)&&(!strlen(row[0]))) ? "NULL" : row[0])) ; 
        puts( "query ok !\n" ) ; 
    } 
